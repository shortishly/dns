...
decode(Resource, Data, Packet) ->
    decode(dns_protocol:qr(Packet), Resource, Data, Packet).

decode(QR, Resource, Data, Packet) ->
    {Name, _} = dns_name:decode(Resource, Packet),
    {Algorithm, <<
                  TimeSigned:48,
                  Fudge:16,
                  MACSize:16,
    ...
    decode(QR, reduce_arcount(Pristine), Name, Algorithm,
           dns_date:to_datetime(TimeSigned), Fudge, MAC, OriginalID,
           dns_rcode:lookup(Error), OtherData).



decode(response, Pristine, Name, Algorithm, TimeSigned,
       Fudge, MAC, OriginalID, Error, OtherData) ->
    Id = dns_protocol:id(Pristine),
...
check_mac(Name, Algorithm, TimeSigned, Fudge, MAC, OriginalID, Error,
    ...
    Elapsed = dns_date:seconds_elapsed_since(TimeSigned),
...
encode(TSIG, Pristine, WithHeader, Offsets) ->
    encode(dns_protocol:qr(Pristine), TSIG, Pristine, WithHeader, Offsets).
...
encode(response, #{algorithm := Algorithm,
       ...
       Offsets1) when Error == bad_sig orelse
                      Error == bad_key ->
    {Packet2, Offsets2} = dns_name:encode(Algorithm, Packet1, Offsets1),
    {<<
       Packet2/binary,
...
encode(response, #{algorithm := Algorithm,
    ...
    Fudge = dns_config:tsig_rr_fudge(),
    Digest = <<
    ...
    {Packet2, Offsets2} = dns_name:encode(Algorithm, WithHeader, Offsets1),
    ...
    true = ets:insert_new(?MODULE,
                          r(dns_protocol:id(Pristine), RequestMAC)),
...
encode(QR, #{algorithm := _,
    ...
    encode(QR, TSIG#{fudge => dns_config:tsig_rr_fudge()}, Pristine,
           WithHeader, Offsets).
...
rdata(TimeSigned, MAC, ID, Error, OtherData) ->
    rdata(TimeSigned, dns_config:tsig_rr_fudge(), MAC, ID, Error, OtherData).

rdata(TimeSigned, Fudge, MAC, OriginalID, Error, OtherData) ->
    <<
      (dns_date:seconds_since_epoch(TimeSigned)):48,
      Fudge:16,
      (byte_size(MAC)):16,
      MAC/binary,
      OriginalID:16,
      (dns_rcode:lookup(Error)):16,
...
variables(Name, Algorithm, TimeSigned, Fudge, Error, OtherData) ->
    <<
      (dns_name:encode(Name))/binary,
      ?ANY:16,
      ?TTL:32,
      (dns_name:encode(Algorithm))/binary,
      (dns_date:seconds_since_epoch(TimeSigned)):48,
      Fudge:16,
      (dns_rcode:lookup(Error)):16,
      (byte_size(OtherData)):16,
