    ...
    lists:foldl(
      fun
          (#{name := Name, class := Class, type := Type}, A) ->
              case dns_res:lookup(Name, Class, Type) of
                  not_found ->
...
encode(#{header := #{id := ID,
    ...
    {HeaderAndQuestions, Offsets} = dns_question:encode(Questions, Header),
    {Packet, _} = dns_rr:encode(HeaderAndQuestions,
                                Offsets,
...
decode(<<
    ...
    {Questions, AnswerRemainder} = dns_rr:questions(QDCOUNT, Remainder,
                                                    Packet),

    {Answers, AuthorityRemainder} = dns_rr:resources(ANCOUNT,
                                                     AnswerRemainder,
                                                     Packet),
    {Authority, AdditionalRemainder} = dns_rr:resources(NSCOUNT,
                                                        AuthorityRemainder,
                                                        Packet),
    {Additional, <<>>} = dns_rr:resources(ARCOUNT, AdditionalRemainder,
                                          Packet),
