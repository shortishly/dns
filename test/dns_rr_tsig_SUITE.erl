...
init_per_suite(Config) ->
    ...
    Origin = [<<"example">>,<<"com">>],
    dns_secret:add(Origin, base64:decode("l8DiN0kqlDhGVfD5wYwfDg==")),
    [{origin, Origin},
...
decode_after_fudge_elased_encode_request_test(Config) ->
    ...
    Encoded = dns_protocol:encode(Packet),
    timer:sleep(2000),
    #{additional := [#{type := tsig,
                       data := Data}]} = dns_protocol:decode(Encoded),
    ?assertMatch(#{mac := _,
                   time_signed := _,
                   error := bad_time}, Data).
...
decode_encode(Packet) ->
    dns_protocol:decode(dns_protocol:encode(Packet)).
