...
simple_at_sign_test(_Config) ->
    {ok, [{origin, 1}], 1} = dns_zone_leexer:string("@").

simple_comment_test(_Config) ->
    {ok, [{comment, 1, " abc"}], 1} = dns_zone_leexer:string("; abc").

simple_escape_test(_Config) ->
    {ok, [{escape, 1, "."}],1} = dns_zone_leexer:string("\\.").

simple_octet_test(_Config) ->
    {ok, [{octet, 1, 83}],1} = dns_zone_leexer:string("\\123").

simple_left_paren_test(_Config) ->
    {ok, [{lparen, 1}],1} = dns_zone_leexer:string("(").

simple_right_paren_test(_Config) ->
    {ok, [{rparen, 1}],1} = dns_zone_leexer:string(")").

simple_integer_test(_Config) ->
    {ok, [{integer, 1, 666}],1} = dns_zone_leexer:string("666").

simple_name_test(_Config) ->
    {ok, [{name, 1, "VENERA"}],1} = dns_zone_leexer:string("VENERA").

simple_control_entity_origin_test(_Config) ->
    {ok,
     [{control, 1, origin}], 1} = dns_zone_leexer:string("$ORIGIN").

simple_control_entity_include_test(_Config) ->
    {ok,
     [{control, 1, include}],1} = dns_zone_leexer:string("$INCLUDE").

simple_control_entity_ttl_test(_Config) ->
    {ok,
     [{control, 1, ttl}],1} = dns_zone_leexer:string("$TTL").
...
sample_027_test(Config) ->
    gold(Config, "sample027").

sample_028_test(Config) ->
    gold(Config, "sample028").
...
parse(Config, Filename) ->
    dns_zone:parse(common:read_file(Config, Filename)).
